...
E_TX = E_th;       %发送数据包需要的能量:400uW * 1ms = 0.4uJ
Emax = 2000;
Bmax = 200;
...
for n=1:N
    ...
    lambdaE = 3*(1/100)*ones(1,N);   %能量包每秒到达数 /slot
%% ------------------------------------------------------------------------
for indE = 1:10%   多种优先级情况下
    lambdaE = lambdaE + 6*(indE/100)*ones(1,N);   %能量包每秒到达数 /slot
    %-----信道模型使用马尔科夫链对信道状态建模，设置信道状态转移概率---------
    Pbg = 0.2*ones(1,N);
    Pgb = 0.2*ones(1,N);   %为0为理想信道条件，信道恒定为GOOD不变
...
% %     hist_Act = zeros(Tsim,N);
    ...
    for j = 1: Tsim
        ...
        last_TX_time_RAP = last_TX_time-(j-1)*TB; % 实际上是要在后续计算中加上一个(j-1)*TB，可读性不好
%         tic
        [ReTX_time_pre,Def_Time_Pre,CSMA_Sta_Pre,PL_RAP,PS_RAP,Colli_RAP,ELE_RAP,Succ_TX_time_RAP,E_buff,B_buff,~,~] = slotCSMACA_unsat_newnoE(len_RAP,CSMA_Sta_Pre,Def_Time_Pre,RAP_CHN_Sta,ReTX_time_pre,CW,last_TX_time_RAP,E_buff,B_buff);%ELE_RAP,（倒数第四）,E_buff,E_flow（最后两个）等一下改 CW可以全局传过去
%         toc
