function [ReTX_time,backoff_after,CSMA_sta,pl_t,ps_t,PL_colli,ELE_ex,TX_time,E_buff,B_buff,Count,ELE_tx] = slotCSMACA_unsat_newnoE(rap_length,CSMA_sta,def_time_pre,last_CHN_sta,ReTX_times_pre,CW,last_TX_time,E_buff,B_buff) %act,
...
Backoff_time = -1*ones(1,N); % temporal variable to record backoff time
CHN = zeros(1,rap_length); % 初始化每个时隙的信道状况为空闲
ELE_ex = zeros(1,N); % 记录本超帧消耗的总能量
ELE_tx = zeros(1,N); % 记录本超帧传输消耗的总能量
backoff_after = def_time_pre; % deferred backoff should be output,default 0
...
%          if (Backoff_time(n) == 1) %yf
    ...
    %检查节点状态
    for n=1:N  
        if( (rap_length-t-T_pkt)>=0 && E_buff(n)>=(E_TX) && B_buff(n)>=1 ) % &&isRAP(n)==1  &&E_buff(n)>=(E_TX)
